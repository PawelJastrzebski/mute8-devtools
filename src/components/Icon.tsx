...
export default function Icon(props: { iconName: IconsNames, size?: number, onClick?: () => void }) {
    const icon = getSvgIcon(props.iconName, props.size ?? 24)
    return (
        <div onClick={props.onClick} class={`icon ${props.iconName}-icon`}>
            {icon}
        </div>
    )
}
